...
```shell
cargo run --bin databroker -- --tls-cert certificates/Server.pem --tls-private-key certificates/Server.key
```

To enable authorization, provide a public key used to verify JWT access tokens (provided by connecting clients).

```shell
cargo run --bin databroker -- --jwt-public-key certificates/jwt/jwt.key.pub --insecure
```
...
```shell
databroker-cli --ca-cert certificates/CA.pem
```
...
```shell
docker build -f Dockerfile -t databroker:<tag> .
```

Use following command if buildplatform is required
```shell
DOCKER_BUILDKIT=1 docker build -f Dockerfile -t databroker:<tag> .
```
