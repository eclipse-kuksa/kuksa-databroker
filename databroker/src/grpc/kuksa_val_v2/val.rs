...
mod tests {
    ...
    */
        ...
        let request = OpenProviderStreamRequest {
            action: Some(open_provider_stream_request::Action::ProvideSignalRequest(
                proto::ProvideSignalRequest {
                    signals_sample_intervals: [(
                        entry_id,
                        proto::SampleInterval { interval_ms: 0 },
                    )]
                    .into_iter()
                    .collect(),
                },
        ...
        match proto::val_server::Val::open_provider_stream(&broker, streaming_request).await {
            Ok(response) => {
                ...
                let result_response = receiver
                    ...
                    .expect("result_response should be Some");
                assert!(
                    result_response.is_err(),
                    "A read-only token must not be allowed to register as signal provider"
                );
                assert_eq!(
                    result_response.unwrap_err().code(),
            ...
            Err(_) => {
                panic!("open_provider_stream itself should not fail")
            }
        }
    }
    ...
    async fn test_provide_signal_with_provide_scope_succeeds() {
        ...
        let entry_id = authorized_access
            .add_entry(
                "test.sensor2".to_owned(),
                broker::DataType::String,
                broker::ChangeType::OnChange,
                broker::EntryType::Sensor,
                "Test sensor 2".to_owned(),
                None,
                None,
                None,
                None,
        ...
        match proto::val_server::Val::open_provider_stream(&broker, streaming_request).await {
            Ok(response) => {
                ...
                let result_response = receiver
                    ...
                    .expect("result_response should be Some");
                assert!(
                    result_response.is_ok(),
                    "A token with provide scope must be allowed to register as signal provider"
                );
                assert!(
                    matches!(
                        result_response.unwrap().action,
                        Some(ProvideSignalResponse(_))
                    ),
                    "Expected ProvideSignalResponse on success"
                );
            }
            Err(_) => {
                panic!("open_provider_stream itself should not fail")
